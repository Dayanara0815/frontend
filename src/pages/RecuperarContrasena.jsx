      ...
      <div style={styles.wrapper}>
        <div style={styles.container}>
          <div style={styles.card}>
            ...
            <p style={styles.info}>Ingresa tu correo y te enviaremos un código de verificación.</p>
            {error && <p style={styles.error}>{error}</p>}
            {enviado && <p style={styles.ok}>¡Código enviado! Redirigiendo...</p>}
            <input style={styles.input} placeholder="Correo electrónico"
              onChange={e => setCorreo(e.target.value)} />
            <button style={styles.btn} onClick={handleEnviar}>Enviar código</button>
            <p style={styles.link} onClick={() => nav('/login')}>Volver al inicio de sesión</p>
          </div>
        </div>
      </div>
      <Footer />
...
const styles = {
  ...
  ok: { color:'#5F7E6D', fontSize:'0.9rem', marginBottom:'10px', background:'#f0fff4', padding:'8px', borderRadius:'8px' }
};